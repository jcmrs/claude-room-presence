...
2. Call `room_join` with the code and your display name
3. After joining, enter Persistent Listen mode:
   - Call `room_listen` after every `room_send`
   - Stay in the loop until the room ends, you are kicked, or the host says to leave
4. Announce your arrival with a brief introduction
5. The room-presence monitor activates automatically via `when: "on-skill-invoke:room-presence"`
